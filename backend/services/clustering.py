...
from models.entity import Cluster, Note

class ClusterService:
    def __init__(self, db: Session):
        self.db = db
        self.DISTANCE_THRESHOLD = 0.40        
        self.MIN_NOTES_PER_CLUSTER = 10       
    ...
    def AssignCluster(self, title: str, content: str, document_vector: list[float]) -> int:
        ...
        distance_col = Cluster.cluster_vector.cosine_distance(document_vector).label("distance")
        stmt = select(Cluster, distance_col).order_by(distance_col).limit(1)
        result = self.db.execute(stmt).first()

        if result and result.distance < self.DISTANCE_THRESHOLD:
            best_cluster = result.Cluster
            
            # Count current notes to calculate the moving average
            current_note_count = self.db.execute(
                select(func.count(Note.id)).where(Note.cluster_id == best_cluster.id)
            ).scalar() or 0
    ...
    def garbage_collect(self, cluster_id: int) -> bool:
        ...
        remaining_note = self.db.execute(
            select(Note).where(Note.cluster_id == cluster_id).limit(1)
        ).scalar_one_or_none()

        if not remaining_note:
            self.db.execute(delete(Cluster).where(Cluster.id == cluster_id))
            self.db.flush()
    ...
    def DivideCluster(self, cluster_id: int) -> bool:
        ...
        cluster = self.db.execute(select(Cluster).where(Cluster.id == cluster_id)).scalar_one_or_none()
        
        if not cluster or len(cluster.notes) < self.MIN_NOTES_FOR_MITOSIS:
            return False 
        ...
        if distance < self.MITOSIS_SPLIT_THRESHOLD:
            print(f"Cluster '{cluster.name}' notes are too similar. Aborting Division")
            return False 
